...
export const forwardRef = <C extends ComponentType<any>>(Component: C) => {
  ...
  const RenderedComponent = Component as ComponentType<
    WithoutForwardRouterProps<Props> & WithoutForwardRouterProps<C>
  >;
  const WithForwardRefComponent = forwardRefFunc(
    (props: Props, ref: React.ForwardedRef<C>) => {
      return <RenderedComponent {...props} ref={ref} />;
    },
  );
  return WithForwardRefComponent as unknown as ComponentType<
    WithoutForwardRouterProps<Props> & WithForwardRefProps<C>
  > &
    C;
};
